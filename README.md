# Starting equipment mod

Simple mod for setup starting equipment & items in freeplay game.

Settings format is list "item=count" or "quality::item=count" separeted by space char ' '.

Example: `'epic::modular-armor=1 personal-roboport-equipment=1 battery-equipment=3 solar-panel-equipment=15 firearm-magazine=50 construction-robot=10'`
...
If quality items availabe you can use quality tag.

For setting string:

`epic::modular-armor=1 rare::stone-furnace=10 burner-mining-drill=20`

For lua remote interface just add quality field value:
...
function setup_starting_items()
  local default_starting_items = {
    ...
    { name = "battery-equipment",                    count = 2                    },
    { name = "apm_zx80_construction_robot",          count = 10                   },
    { name = "modular-armor",                        count = 1,    quality="epic" },
    { name = "apm_assembling_machine_0",             count = 5                    },
    { name = "firearm-magazine",                     count = 50                   },
  }
